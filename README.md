...
| 순서 | 흐름 | 내용 |
|---|---|---|
| 1 | 메인 스레드 → 제안자 | 지시 + 메인이 선택한 컨텍스트 (선호 자동 제거 없음) |
| 2 | 제안자 → 메인 스레드 | 구현안 + 가정 태그 |
| 3 | 메인 스레드 → 감사자 | 지시 + 제안자의 안 |
| 4 | 감사자 → 메인 스레드 | 6축 판정 + 실패 시나리오 |
| 5 | 메인 스레드 | 합의는 확정하고 충돌만 질문 |

### `fight-clarify`

| 순서 | 흐름 | 내용 |
|---|---|---|
| 1 | 메인 스레드 → 해석자 A | 지시 + 극단 A 배정 |
| 2 | 메인 스레드 → 해석자 B | 지시 + 극단 B 배정 (1과 같은 메시지에서 병렬) |
| 3 | 해석자 A → 메인 스레드 | 구현안 A |
| 4 | 해석자 B → 메인 스레드 | 구현안 B |
| 5 | 메인 스레드 | 일치점은 확정하고 갈린 점만 질문 |
...
서브에이전트 오케스트레이션 단위 테스트는 없다. 호출을 실행하는 로직이 아니라 호스트 지시문이기 때문이다. 검증은 계획 문서의 시나리오 1–3을 실제 호출로 재현한다.
