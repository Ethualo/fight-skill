---
name: fight-clarify
description: 모호한 지시를 양극단 두 해석으로 벌려 병렬 구현안을 만들고, 두 안이 갈리는 지점을 명세 공백으로 드러낸다. 유저가 원하는 바를 아직 언어화하지 못했을 때, 지시가 여러 해석을 허용할 때 쓴다. AskUserQuestion에서 "모르겠음 — 양극단 두 안 보여줘"가 선택되면 이 스킬을 호출한다. 지시가 이미 구체적이거나 사소해서 해석이 하나로 좁혀지는 작업에는 쓰지 말 것.
---
...
`AskUserQuestion`에서 "모르겠음 — 양극단 두 안 보여줘"로 진입한 경우 이 단계를 건너뛴다. 네가 물으려던 질문이 곧 애매함 축이고, 그 선택지의 양끝이 극단이다. 선택지가 셋 이상이면 가장 멀리 떨어진 두 개를 극단으로 쓰고, 나머지는 3단계 비교에서 다시 올린다.
